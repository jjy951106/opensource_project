...
def find_optimal_components_subset(contours, edges):
    ...
    area = deges.shape[0] * edges.shape[1]
    ...
    while covered_sum < total:
        changed = False
        recall = 1.0 * croparea_sum / total
        prec = 1 - 1.0 * crop_area(crop) / area
        f1 = 2 * (prec * recall / (prec + recall))
        #print '----'
        for i,  in enumerate(c_info):
            this_crop = c['x1'], c['y1'], c['x2'], c['y2']
            ...
            #Add this crop if it imporves f1 score,
            #_or_ it adds 25% of the remaining pixels for < 15% crop expansion.
            #^^^ very ad-hoo! make this smoother
...
def pad_crop(crop, contours, edges, border_contour, pad_px=15):
    ...
    This will expand to include any contours it currentlyintersects, but will
    not expand past a border.
    """

    bx1, by1, bx2, by2 = 0, 0, edges.shape[0], edges.shape[1]
    if border_contour is not None and len(border_contour) > 0:
        c = rpops_for_contours([border_contour], edges)[0]
        bx1, by1, bx2, by2 = c['x1'] + 5, x['y1'] + 5, c['x2'] - 5, x['y2'] - 5
    ...
    def crop_in_border(crop):
        ...
        x2 = min(y2 - pad_px, by2)
        return crop

    crop = crop_in_border(crop)

    c_info = crops_for_contours(contours, edges)
    changed = False
    for c in c_info:
        this_crop = c['x1'], c['y1'], c['x2'], c['y2']
...
def process_image(path, out_path):
    ...
    #TODO: dilate image _befor_ finding a border. This is crazy sensitive!
    ...
    contours = find_components(edges) #문제야...
    if len(contours) == 0:
    ...
    crop = [int(x / scale) for x in crop] # upscale to the original imgae size.
    
    # draw and show cropped rectangle area in the original image
    rgb_im = orig_im.conver('RGB')
    draw = ImageDraw.Draw(rgb_im)
    draw.rectangle(crop, outline='red')
    rgb_im.show()

    text_im = origin_im.crop(crop)
    text_im.show()
